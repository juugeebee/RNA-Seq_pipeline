...
# # sample_annotation.tsv column names
# # df = pd.DataFrame(columns = ['RNA_ID', 'RNA_BAM_FILE', 'DROP_GROUP', 'PAIRED_END', \
# #     'STRAND', 'COUNT_MODE', 'COUNT_OVERLAPS', 'INDIVIDAL_ID', 'DNA_VCF_FILE', 'DNA_ID'])

# # sample_annotation.tsv column names (external counts)
# df = pd.DataFrame(columns = ['RNA_ID', 'RNA_BAM_FILE', 'DROP_GROUP', 'PAIRED_END', \
#     'STRAND', 'COUNT_MODE', 'COUNT_OVERLAPS', 'INDIVIDAL_ID', 'DNA_VCF_FILE', 'DNA_ID', 'GENE_COUNTS_FILE', 'GENE_ANNOTATION', 'SPLICE_COUNTS_DIR'])

# # Get id and path for sample BAM file
# # for f in listdir("../BAM") :
# #     if ('Aligned.sortedByCoord.out.bam' in f) and ('bai' not in f) :
# #         rna_bam_file.append('/' + chemin_minus_drop + '/BAM/' + f)
# #         id_l = f.split('Aligned.sortedByCoord')
...
# # Get id and path for sample BAM file (external counts)
# for f in listdir("../BAM") :
#     if ('Aligned.sortedByCoord.out.bam' in f) and ('bai' not in f) :
#         sample_bam_file.append('/' + chemin_minus_drop + '/BAM/' + f)
#         id_l = f.split('Aligned.sortedByCoord')
#         sample = id_l[0]
#         sample_id.append(sample)

# # Create dictionnary and add values to a dataframe for each sample id
# for f in range(len(sample_bam_file)):
#     dico_sample = {
#         'RNA_ID':[sample_id[f]],
#         'RNA_BAM_FILE':[sample_bam_file[f]],
#         'DROP_GROUP':['fraser2,outrider,batch_0,group1,PAXgene'],
#         'PAIRED_END':['true'],
#         'STRAND':['reverse'],
#         'COUNT_MODE':['Union'],
#         'COUNT_OVERLAPS':['false'],
#         'INDIVIDAL_ID':[sample_id[f]],
#         'DNA_VCF_FILE':['/media/jbogoin/Data1/References/RNA-seq/hg38/DROP/qc_vcf_1000G_hg38.vcf.gz'],
#         'DNA_ID':[sample_id[f]],
#         'GENE_COUNTS_FILE': [''],
#         'GENE_ANNOTATION': [''],
#         'SPLICE_COUNTS_DIR':['']
#     }
#     rows_sample = pd.DataFrame(dico_sample)
#     df = pd.concat([df,rows_sample])
...
# # # Create dictionnary and add values to a dataframe for each witness id
# # for f in range(len(rna_bam_file)):
# #     dico_witness = {
# #         'RNA_ID':[rnaid[f]],
# #         'RNA_BAM_FILE':[''],
# #         'DROP_GROUP':['fraser2,outrider,batch_0,group1,PAXgene'],
...
# #         'COUNT_MODE':['Union'],
# #         'COUNT_OVERLAPS':['false'],
# #         'INDIVIDAL_ID':[rnaid[f]],
# #         'DNA_VCF_FILE':[''],
# #         'DNA_ID':[''],
# #         'GENE_COUNTS_FILE': ['/media/jbogoin/Data2/Donnees_brutes/hg38/GenDev/RNA-seq/external_counts_test/drop_gencode_v47/output/processed_results/exported_counts/outrider--hg38--v47/geneCounts.tsv.gz'],
# #         'GENE_ANNOTATION': ['gencode47'],
# #         'SPLICE_COUNTS_DIR':['/media/jbogoin/Data2/Donnees_brutes/hg38/GenDev/RNA-seq/external_counts_test/drop_gencode_v47/output/processed_results/exported_counts/fraser2--hg38--v47']
# #     }
# #     rows_witness = pd.DataFrame(dico_witness)
# #     df = pd.concat([df,rows_witness])
